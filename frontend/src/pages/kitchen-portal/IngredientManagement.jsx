...
export default function IngredientManagement() {
  const [isProfileModalOpen, setIsProfileModalOpen] = useState(false);
    ...
    // States Modal (Popup thêm/sửa)
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingId, setEditingId] = useState(null);
    const [formData, setFormData] = useState({
        Name: '', Category: 'Thịt & Cá', StockQuantity: 0, Unit: 'kg', Supplier: '', MinStockLevel: 5
    });
    ...
    const categories = ['Thịt & Cá', 'Rau củ quả', 'Gia vị', 'Đồ khô', 'Sữa & Nước', 'Khác'];
    const units = ['kg', 'gram', 'lít', 'ml', 'chai', 'gói', 'thùng', 'quả'];
    ...
    const handleAddNew = () => {
        setEditingId(null);
        setFormData({ Name: '', Category: 'Thịt & Cá', StockQuantity: 0, Unit: 'kg', Supplier: '', MinStockLevel: 5 });
        setIsModalOpen(true);
    };
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (editingId) {
                ...
                showToast("Cập nhật kho thành công!", "success"); // [MỚI THÊM] Đổi từ alert sang showToast
            } else {
                await api.post('/kitchen/ingredients', formData);
                showToast("Đã thêm nguyên liệu mới!", "success"); // [MỚI THÊM] Đổi từ alert sang showToast
            }
            setIsModalOpen(false);
            fetchIngredients();
        } catch (error) {
            showToast("Có lỗi xảy ra, vui lòng thử lại!", "error"); // [MỚI THÊM] Đổi từ alert sang showToast
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            showToast("Đã xóa nguyên liệu khỏi kho!", "success"); // [MỚI THÊM]
            fetchIngredients();
        } catch (error) {
            showToast("Lỗi khi xóa dữ liệu!", "error"); // [MỚI THÊM]
        }
    };
        ...
        <div className="min-h-screen bg-[#f9fafb] flex flex-col  text-gray-800 pb-10 relative">
            ...
            <nav className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-30">
                <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    ...
                    <div className="hidden lg:flex space-x-6 text-sm font-semibold text-gray-500 h-full">
                        ...
                        <Link to="/kitchen/create-menu" className="hover:text-orange-600 h-full flex items-center transition-colors">Tạo menu ngày</Link>
            ...
            <div className="bg-white border-b border-gray-200 shadow-sm z-20">
                <div className="max-w-[1500px] mx-auto px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">Kiểm kê kho nguyên liệu</h1>
                        <p className="text-sm text-gray-500 font-medium mt-1">Theo dõi tồn kho, nhập xuất và cảnh báo hàng sắp hết.</p>
                    </div>

                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-64">
                            <Search className="absolute left-3 top-2.5 text-gray-400" size={18} />
                            <input type="text" placeholder="Tìm tên, NCC..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded text-sm focus:ring-2 focus:ring-orange-500 outline-none" />
                        </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200 text-xs uppercase font-black text-gray-500 tracking-wider">
                                    <th className="p-4 w-1/3">Tên nguyên liệu</th>
                            ...
                            <tbody className="divide-y divide-gray-100 text-sm">
                                {isLoading ? (
                                    ...
                                    <tr><td colSpan="6" className="p-8 text-center text-gray-500 font-medium">Không tìm thấy nguyên liệu nào!</td></tr>
                                ) : (
                                    filteredIngredients.map((item) => {
                                        ...
                                        return (
                                            <tr key={item.id || item.IngredientID} className="hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="p-4 text-gray-600">{item.Supplier || 'Chưa rõ'}</td>
                    ...
                    <div className="bg-white rounded-md shadow-sm w-full max-w-md overflow-hidden">
                        <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50">
                            <h3 className="font-bold text-lg text-gray-900 flex items-center gap-2">
                                <Package className="text-orange-500" size={20} /> {editingId ? 'Cập nhật nguyên liệu' : 'Nhập kho mới'}
                            </h3>
                            <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-700"><X size={20} /></button>
                        </div>
                        <form onSubmit={handleSave} className="p-5 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Tên nguyên liệu *</label>
                                <input required type="text" name="Name" value={formData.Name} onChange={handleChange} className="w-full p-2.5 border border-gray-200 rounded focus:ring-2 focus:ring-orange-500 outline-none text-sm font-medium" placeholder="VD: Cà chua Đà Lạt" />
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Phân loại</label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Đơn vị</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Số lượng hiện tại *</label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Cảnh báo sắp hết</label>
                                    <input type="number" step="0.1" name="MinStockLevel" value={formData.MinStockLevel} onChange={handleChange} className="w-full p-2.5 border border-gray-200 rounded outline-none text-sm font-medium text-red-500" title="Nếu tồn kho dưới mức này sẽ hiện cảnh báo đỏ" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-gray-700 uppercase mb-1.5">Nhà Cung Cấp</label>
                                <input type="text" name="Supplier" value={formData.Supplier} onChange={handleChange} className="w-full p-2.5 border border-gray-200 rounded focus:ring-2 focus:ring-orange-500 outline-none text-sm font-medium" placeholder="VD: Vinmart, Co.opmart..." />
                            </div>

                            <div className="pt-4 flex gap-3">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 px-4 py-2.5 border border-gray-200 text-gray-700 rounded font-bold hover:bg-gray-50 transition-colors">Hủy</button>
                                <button type="submit" className="flex-1 px-4 py-2.5 bg-gray-900 text-white rounded font-bold hover:bg-gray-800 transition-colors shadow-sm">Lưu</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {/* ==========================================
                [MỚI THÊM] POPUP THÔNG BÁO (TOAST)
                ========================================== */}
            {toast.show && (
