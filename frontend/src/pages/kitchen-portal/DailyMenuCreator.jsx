...
export default function DailyMenuCreator() {
  ...
  const minDate = useMemo(() => {
    ...
    // Fix Timezone: Get local date correctly instead of using toISOString() (which might regress to Sunday in UTC)
 ...
 */
      ...
      try {
        ...
        if (res.data && res.data.data) {
          ...
          if (fetchedMenu.Status === 'Rejected') {
            setRejectReason(fetchedMenu.RejectReason || 'Menu needs nutritional adjustment.');
          }
  ...
  const handleCheckAllergy = async () => {
    if (!menuDate) return alert("Please select a date!");
    setIsChecking(true);
    try {
      ...
      if (res.data.status === 'SAFE') alert("Menu is safe.");
    } catch (error) {
      alert("Allergy check error.");
  ...
  const handleSaveMenu = async () => {
    ...
    try {
      ...
      setStatus('Submitted');
      showToast("Menu sent for approval!");
    } catch (error) {
      showToast(error.response?.data?.message || "Error saving menu.", "error");
        ...
        <div className="flex flex-col bg-gray-50 rounded border border-gray-200 p-4 shadow-sm h-full opacity-90">
          ...
          <div className="bg-white p-3 rounded-lg border border-gray-200 flex-1 flex flex-col justify-center">
            {currentDish ? (
              ...
              <p className="text-gray-400 italic text-sm text-center">Waiting for dish update</p>
            )}
          </div>
        </div>
      );
      ...
      <div className={`flex flex-col bg-white rounded border-2 p-4 shadow-sm h-full transition-all ${!currentDish ? 'border-dashed border-gray-300 bg-gray-50/30' :
        ...
        {currentDish && (
          <div className="mt-2 text-[11px] space-y-1">
            <p className="font-bold text-gray-700">Calories: <span className="text-orange-600">{currentDish.calories} kcal</span></p>
            <p className="text-gray-500 truncate">Supplier: {currentDish.supplier}</p>
          </div>
        )}
      </div>
    );
      ...
      <nav className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-30">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden lg:flex space-x-6 text-sm font-semibold text-gray-500 h-full">
            ...
            <span onClick={() => { navigate('/kitchen/create-menu'); setMenuDate(new Date().toISOString().split('T')[0]); }} className="text-orange-600 border-b-2 border-orange-500 h-full flex items-center cursor-pointer">Daily Menu</span>
            <span onClick={() => navigate('/kitchen/weekly-menu')} className="hover:text-orange-600 h-full flex items-center cursor-pointer transition-colors">
              Weekly Menu
            </span>            <Link to="/kitchen/dishes" className="hover:text-orange-600 h-full flex items-center transition-colors">Dishes</Link>
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm z-20">
        <div className="max-w-[1400px] mx-auto px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Daily Menu Creator</h1>
            </div>
          ...
          <div className="flex items-center w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
            <div className="flex items-center gap-2 sm:gap-3 bg-gray-50/80 p-2 rounded-md border border-gray-100">
              ...
              <div className={`px-4 py-2 rounded text-sm font-bold flex items-center transition-all shadow-sm ${status === 'Approved' ? 'bg-green-100 text-green-700 border border-green-200' :
                ...
                3. {status === 'Rejected' ? 'Rejected' : 'Completed'}
              </div>
      ...
      {status === 'Rejected' && (
        <div className="max-w-[1400px] mx-auto px-6 mt-4">
          <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl flex items-start gap-3 shadow-sm">
            ...
            <div><h3 className="font-bold text-red-800">Rejected:</h3><p className="text-red-700 text-sm">{rejectReason}</p></div>
          </div>
        </div>
      )}

      {/* 3. MEAL REGISTRATION STATS (Only shows when viewed from Weekly Menu) */}
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 mt-4">
          <div className="bg-white border border-indigo-100 rounded-lg p-4 shadow-sm flex flex-wrap items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-2">
              <span className="font-bold text-gray-700">Total Registered:</span>
            ...
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-600">Vegetarian:</span>
              <span className="text-green-600 font-bold">{mealCounts.Vegetarian}</span>
            </div>
            <div className="hidden sm:block w-px h-6 bg-gray-200"></div>
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-500">Not eating:</span>
      ...
      <main className="flex-1 px-4 sm:px-6 py-6 flex flex-col xl:flex-row gap-6 max-w-[1400px] mx-auto w-full">
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-orange-50/40 border border-orange-100 p-5 rounded-md flex flex-col gap-4">
            <h2 className="text-lg font-black text-orange-800 flex items-center border-b border-orange-200/60 pb-3 uppercase">Standard Meal</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DishBlock title="Main Dish" slotKey="std_main" filterType="Standard" colorObj={{ text: 'text-orange-900', icon: 'text-orange-500' }} />
              <DishBlock title="Vegetable" slotKey="std_veg" filterType="Veggie" colorObj={{ text: 'text-green-900', icon: 'text-green-500' }} />
              <DishBlock title="Soup" slotKey="std_soup" filterType="Soup" colorObj={{ text: 'text-blue-900', icon: 'text-blue-500' }} />
              <DishBlock title="Dessert" slotKey="std_side" filterType="Side" colorObj={{ text: 'text-purple-900', icon: 'text-purple-500' }} />
            </div>
          </div>
          {/* VEGETARIAN COLUMN */}
          <div className="bg-green-50/40 border border-green-100 p-5 rounded-md flex flex-col gap-4">
            <h2 className="text-lg font-black text-green-800 flex items-center border-b border-green-200/60 pb-3 uppercase">Vegetarian Meal</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DishBlock title="Main Dish" slotKey="veg_main" filterType="Vegan" colorObj={{ text: 'text-green-900', icon: 'text-green-500' }} />
              <DishBlock title="Vegetable" slotKey="veg_veg" filterType="Veggie" colorObj={{ text: 'text-emerald-900', icon: 'text-emerald-500' }} />
              <DishBlock title="Soup" slotKey="veg_soup" filterType="Soup" colorObj={{ text: 'text-teal-900', icon: 'text-teal-500' }} />
              <DishBlock title="Dessert" slotKey="veg_side" filterType="Side" colorObj={{ text: 'text-purple-900', icon: 'text-purple-500' }} />
            </div>
          </div>
        </div>
        ...
        <div className="w-full xl:w-[360px] flex flex-col gap-5">
          <div className="bg-[#fff9e6] border border-[#fce49c] rounded-md p-5 shadow-sm">
            <h2 className="text-sm font-bold text-yellow-800 uppercase tracking-wider mb-3">Nutrition (kcal)</h2>
            <div className="space-y-3">
              <div className="flex justify-between font-bold"><span>Standard:</span><span className="text-orange-600">{stdCalories}</span></div>
              <div className="flex justify-between font-bold"><span>Vegetarian:</span><span className="text-green-600">{vegCalories}</span></div>
            </div>
          </div>
          {allergyWarnings.length > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-md p-4 max-h-60 overflow-y-auto">
              <h2 className="text-xs font-bold text-red-800 uppercase mb-2">Allergy Warnings ({allergyWarnings.length})</h2>
              {allergyWarnings.map((w, idx) => (
                <div key={idx} className="text-[11px] mb-2 p-2 bg-white rounded border border-red-100">
                  ...
                  <p className="text-red-600">Conflict: {w.conflictAllergens.join(', ')}</p>
                </div>
              ))}
            </div>
          )}
            ...
            {!isReadOnly && (
              <>
                <button onClick={handleCheckAllergy} disabled={!canSubmit || isChecking} className="w-full bg-gray-900 text-white font-bold py-3.5 rounded hover:bg-gray-800 flex justify-center items-center gap-2">
                  {isChecking ? <Loader2 className="animate-spin" /> : <Search size={18} />} Allergy Check
                </button>
                <button onClick={handleSaveMenu} disabled={!canSubmit || isSaving} className={`w-full font-bold py-3.5 rounded flex justify-center items-center gap-2 ${canSubmit ? 'bg-orange-600 text-white' : 'bg-gray-200 text-gray-400'}`}>
                  {isSaving ? <Loader2 className="animate-spin" /> : <Send size={18} />} Submit Menu
                </button>
              </>
            )}
            {isReadOnly && (
              <div className="w-full bg-blue-50 text-blue-700 font-bold py-4 rounded text-center border border-blue-200">Locked (Pending)</div>
            )}
          </div>
        </div>
      </main>
