...
export default function DishManagement() {
  ...
  // ==========================================
  // DYNAMIC INGREDIENT LIST MANAGEMENT STATE
  // ==========================================
  ...
  const dishCategories = ['Standard', 'Vegan', 'Veggie', 'Soup', 'Side'];
  ...
  const handleOpenAddModal = () => {
    setIsEditing(false);
    setFormData({ id: null, name: '', nameEn: '', type: 'Standard', calories: '', allergies: '' });
    setIngredientList([{ name: '', supplier: '' }]); // Reset list to 1 empty row
    setImageFile(null);
    setPreviewUrl('');
    setIsModalOpen(true);
  };
  ...
  const handleOpenEditModal = (dish) => {
    ...
    // Parse ingredients from Backend into input rows
  ...
  const handleRemoveIngredientRow = (index) => {
    ...
    setIngredientList(newList.length ? newList : [{ name: '', supplier: '' }]); // Keep at least 1 row
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Combine rows into a single string to send to DB
      const combinedIngredients = ingredientList
        .filter(item => item.name.trim() !== '') // Skip empty names
        .map(item => {
          if (item.supplier.trim() !== '') return `${item.name.trim()} - ${item.supplier.trim()}`;
          return item.name.trim(); // If no supplier, send only name
        })
        .join('\n'); // Join with newlines

      formDataToSend.append('ingredients', combinedIngredients);
      formDataToSend.append('supplier', 'Details in Ingredients section');
  ...
  const renderTypeBadge = (type) => {
    const styles = {
      'Standard': 'bg-orange-50 text-orange-700 border-orange-100',
      'Vegan': 'bg-green-50 text-green-700 border-green-100',
      'Veggie': 'bg-green-50 text-green-700 border-green-100',
      'Soup': 'bg-blue-50 text-blue-700 border-blue-100',
      'Side': 'bg-yellow-50 text-yellow-700 border-yellow-100',
    };
    return <span className={`px-3 py-1 rounded-full text-xs font-bold border ${styles[type] || 'bg-gray-50 text-gray-700'}`}>{type}</span>;
  };
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb]  text-gray-800 pb-12 relative">
      ...
      {/* 2. TOOLBAR (Synchronized format with Ingredient Management) */}
      <div className="bg-white border-b border-gray-200 shadow-sm z-20">
        <div className="max-w-[1500px] mx-auto px-6 py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium mt-1">Library of standard dishes to set up daily menus.</p>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-500 uppercase bg-gray-50/50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold">Calories</th>
                  <th className="px-6 py-4 font-bold">Ingredients & Origins</th>
                  <th className="px-6 py-4 font-bold">Allergies</th>
                    ...
                    <tr key={dish.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right align-top">
                        <div className="flex items-center justify-end space-x-2">
                          <button onClick={() => handleOpenEditModal(dish)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><Edit size={16} /></button>
                          <button onClick={() => handleDelete(dish.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-400">
                        ...
                        <p className="text-base font-medium text-gray-600">No dishes found in the library.</p>
                      </div>
                    </td>
                  </tr>
                )}
              ...
              <form id="dishForm" onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full rounded border-gray-200 bg-gray-50 p-3 text-sm focus:ring-orange-500 focus:border-orange-500 transition-colors" placeholder="Ex: Braised Pork" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1.5">English Name</label>
                    <input type="text" value={formData.nameEn} onChange={(e) => setFormData({ ...formData, nameEn: e.target.value })} className="w-full rounded border-gray-200 bg-gray-50 p-3 text-sm focus:ring-orange-500 focus:border-orange-500 transition-colors" placeholder="Ex: Braised Pork" />
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1.5">Dish Type *</label>
                    <select required value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full rounded border-gray-200 bg-gray-50 p-3 text-sm focus:ring-orange-500 focus:border-orange-500 font-medium text-gray-700">
                      ...
                      <option value="Side">Side / Dessert</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1.5">Estimated Calories (kcal) *</label>
                    <input type="number" required min="0" value={formData.calories} onChange={(e) => setFormData({ ...formData, calories: e.target.value })} className="w-full rounded border-gray-200 bg-gray-50 p-3 text-sm focus:ring-orange-500 focus:border-orange-500 transition-colors" placeholder="Ex: 250" />
                  </div>
                </div>
                      ...
                      <div key={index} className="flex items-center gap-3">
                        <div className="flex-1">
                          <input
                            type="text"
                            required
                            placeholder="Ingredient (E.g: Beef)"
                            value={item.name}
                        ...
                        <div className="flex-1">
                          <input
                            type="text"
                            placeholder="Supplier (E.g: CoopMart)"
                            value={item.supplier}
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1.5 flex items-center">
                    Allergy Warnings
                    <span className="ml-2 text-xs font-normal text-pink-500 bg-pink-50 px-2 py-0.5 rounded-full">(Comma separated)</span>
                  </label>
                  <input type="text" value={formData.allergies} onChange={(e) => setFormData({ ...formData, allergies: e.target.value })} className="w-full rounded border-gray-200 bg-gray-50 p-3 text-sm focus:ring-orange-500 focus:border-orange-500 transition-colors" placeholder="E.g: Peanuts, Soy, Seafood" />
                </div>
