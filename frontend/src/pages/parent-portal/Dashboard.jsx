...
export default function Dashboard() {
  ...
  // STATE FOR CHART DATA FROM BACKEND
            ...
            response.data.data.map(async (st) => {
              try {
                ...
                return {
                  ...
                  class: d.ClassName || 'Not assigned',
                  height: d.Height || st.Height || 0,
                  weight: d.Weight || st.Weight || 0,
                  // ALLERGY FIX: Scan both APIs and check all case styles
                  ...
                  registrationStatus: d.registrationStatus || 'Not registered', 
  ...
  // CALL API TO FETCH CHARTS WHEN STUDENT CHANGES
  useEffect(() => {
    ...
    const fetchStudentStats = async () => {
      try {
        ...
        console.log("Backend does not have chart data for this student yet.");
  ...
  if (loading) return <div className="min-h-screen bg-green-50 flex items-center justify-center">Loading data...</div>;
      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex space-x-8 text-sm font-medium text-gray-500">
            <span className="text-green-600 border-b-2 border-green-500 py-5 cursor-pointer">Overview</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-md p-6 shadow-sm border border-gray-100 flex items-center space-x-6">
            ...
            <div className="flex-1">
              <h2 className="text-xl font-bold text-gray-900">{selectedStudent?.name || 'Updating...'}</h2>
              <p className="text-gray-500 text-sm mt-1">Class {selectedStudent?.class} • Tran Dai Nghia Primary School</p>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {selectedStudent?.hasAllergy ? (
                  <div className="inline-flex items-center space-x-1.5 bg-pink-50 text-pink-700 px-3 py-1 rounded-full text-xs font-medium border border-pink-100 shadow-sm">
                    <Shield size={12} />
                    <span>Allergy: {selectedStudent?.allergyNote || 'Recorded'}</span>
                  </div>
                ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100 flex items-center space-x-4">
            ...
            <div>
              <p className="text-sm text-gray-500 font-medium">Eaten this week</p>
              <p className="text-2xl font-bold text-gray-900">{barData.reduce((acc, curr) => acc + curr.meals, 0)} / 5 meals</p>
            </div>
          </div>
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100 flex items-center space-x-4">
            ...
            <div>
              ...
              <p className={`text-lg font-bold ${
                selectedStudent?.registrationStatus === 'Not registered' ? 'text-red-500' : 
                selectedStudent?.registrationStatus === 'Pending Payment' ? 'text-orange-500' : 'text-green-600'
              }`}>
                {selectedStudent?.registrationStatus === 'Chưa đăng ký' ? 'Not registered' : 
                 selectedStudent?.registrationStatus === 'Chờ thanh toán' ? 'Pending Payment' : 
                 selectedStudent?.registrationStatus === 'Đã thanh toán' ? 'Paid' : selectedStudent?.registrationStatus}
              </p>
            </div>
          </div>
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100 flex items-center space-x-4">
            ...
            <div>
              <p className="text-sm text-gray-500 font-medium">Meal Balance</p>
              <p className="text-2xl font-bold text-gray-900">{selectedStudent?.totalMealsThisMonth || 0} meals</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100">
            <h3 className="text-sm font-semibold text-gray-900 mb-6">Eating History (This Week)</h3>
            <div className="h-48">
              {barData.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-sm text-gray-400">No eating data for this week</div>
              )}
            </div>
          </div>

          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100">
            <h3 className="text-sm font-semibold text-gray-900 mb-2">Meal Type Distribution</h3>
            <div className="h-48 flex items-center justify-center">
              {pieData.length > 0 ? (
                ...
                <div className="text-sm text-gray-400">No meal statistics available</div>
              )}
            </div>
            {pieData.length > 0 && (
          ...
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100">
            <h3 className="text-sm font-semibold text-gray-900 mb-6">Monthly Registration Trend</h3>
            <div className="h-48">
              {lineData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-50 flex justify-between items-center">
            <h3 className="text-base font-semibold text-gray-900">Upcoming Meals</h3>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-500 uppercase bg-gray-50/50">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium">Menu Type</th>
                ...
                {upcomingMeals.length > 0 ? (
                    ...
                    <tr key={meal.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          meal.status === 'Đã chốt' || meal.status === 'Finalized' ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'
                        }`}>
                          {meal.status === 'Đã chốt' ? 'Finalized' : meal.status}
                        </span>
                      </td>
                  ...
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center text-gray-400 italic">
                      No upcoming meals. Please register and select dishes.
                    </td>
                  </tr>
                )}
              </tbody>
